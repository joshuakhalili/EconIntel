-- Metadata is evidence even before the first observation arrives. Exclude
-- operational timestamps/refresh policy so an unchanged fetch is not a review.
...
SELECT q.id AS question_id,
  jsonb_build_object(
    ...
    'indicator_definitions', definitions.snapshot,
...
LEFT JOIN LATERAL (
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'indicator', to_jsonb(i) - ARRAY['created_at','updated_at','last_ingested_at','refresh_interval'],
    'source', jsonb_build_object('id', s.id, 'name', s.name,
      'homepage_url', s.homepage_url, 'licence', s.licence,
      'attribution_text', s.attribution_text, 'credibility', s.credibility,
      'notes', s.notes)
  ) ORDER BY i.id), '[]'::jsonb) AS snapshot
  FROM indicators i LEFT JOIN sources s ON s.id = i.source_id
  WHERE EXISTS (SELECT 1 FROM question_indicators qi
    WHERE qi.question_id = q.id AND qi.indicator_id = i.id)
) definitions ON true
LEFT JOIN LATERAL (
  SELECT md5(COALESCE(string_agg(jsonb_build_array(
    ...
    o.confidence_tier, i.unit, i.cadence, i.description
